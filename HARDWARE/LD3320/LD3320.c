					 ...
					 /*switch(nAsrRes)		   			//对结果执行相关操作,客户修改
						{
							...
							case CODE_GD:	 					//命令“关灯”
								//printf(" 关灯 指令识别成功\r\n"); 
															 break;
							case CODE_DLD:					//命令“低亮度”
								//printf(" 低亮度 指令识别成功\r\n"); 
															break;
							case CODE_ZCLD:						//命令“正常亮度”
								//printf(" 正常亮度 指令识别成功\r\n");
															break;
							case CODE_GLD:						//命令“高亮度”
								//printf(" 高亮度 指令识别成功\r\n");
															break;
							case CODE_LSW:						//命令“冷色温”
								//printf(" 冷色温 指令识别成功\r\n");
															break;
							case CODE_ZCSW:						//命令“正常色温”
								//printf(" 正常色温 指令识别成功\r\n");
															break;
							case CODE_NSW:						//命令“暖色温”
								//printf(" 暖色温 指令识别成功\r\n");
															break;
							case CODE_KQLY:						//命令“开启蓝牙”
								//printf(" 开启蓝牙 指令识别成功\r\n");
															break;
							case CODE_GBLY:						//命令“关闭蓝牙”
								//printf(" 关闭蓝牙 指令识别成功\r\n");
															break;
							default:break;
						}	*/
...
static void KD(void)  //开灯
{
	LED1_OFF();
	led_R_pwmval=255;
	led_G_pwmval=255;
	led_B_pwmval=255;
	TIM_SetCompare1(TIM1,led_R_pwmval);	
	TIM_SetCompare4(TIM1,led_G_pwmval);
	TIM_SetCompare3(TIM1,led_B_pwmval);

}

static void GD(void)  //关灯
{
	LED1_OFF();
	led_R_pwmval=0;
	led_G_pwmval=0;
...
static void KQLY(void)  //开启蓝牙
{
	LED1_ON();
		if(USART2_RX_STA&0X8000)	//接收到一次数据了
		{
			//printf("蓝牙传输数据 ： %s\r\n",USART2_RX_BUF);
			if((USART2_RX_BUF[0]=='O')&&(USART2_RX_BUF[1]=='N')&&(USART2_RX_BUF[2]==')'))
				{
					led_R_pwmval=255;
					led_G_pwmval=255;
					led_B_pwmval=255;
				}
				else if((USART2_RX_BUF[0]=='O')&&(USART2_RX_BUF[1]=='F')&&(USART2_RX_BUF[2]=='F')&&(USART2_RX_BUF[3]==')'))
				{
					led_R_pwmval=0;
					led_G_pwmval=0;
					led_B_pwmval=0;
				...
				{
					led_R_pwmval=RGB_Convert(USART2_RX_BUF,1);
					led_G_pwmval=RGB_Convert(USART2_RX_BUF,2);
					led_B_pwmval=RGB_Convert(USART2_RX_BUF,3);
				}
...
{	
		...
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
		GPIO_Init(BT_GPIO_PORT, &GPIO_InitStructure);
